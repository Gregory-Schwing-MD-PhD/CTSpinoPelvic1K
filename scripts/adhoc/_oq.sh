...
#SBATCH --output=logs/openq2_%j.out
#SBATCH --error=logs/openq2_%j.err
set -euo pipefail
...
singularity exec --bind "$(pwd)":/w,"${DATA_DIR}":/data --pwd /w \
    ...
    python3 -u /w/scripts/measure_open_questions.py \
        --src "${DATA_DIR}/hf_export_v6" --out "${DATA_DIR}/open_questions.json"
