...
def title(ax, main, sub=None):
    ax.set_title(main, loc="left", pad=24 if sub else 6)
    if sub:
        ax.text(0, 1.045, sub, transform=ax.transAxes, fontsize=8, color=MUTE,
                va="bottom", ha="left")
...
def p_hist(ax, rows, key, label, sub, colour, expect=None):
    ...
    if a.size == 0:
        ax.axis("off"); return
    # integer bins only for genuinely INTEGER data. rib12/rib11 spans 0.10-1.02 and got
    # two bars because the test asked whether the RANGE was narrow, not whether the values
    # were whole numbers -- and a ratio bounded by 0..2 passes "<= 14" every time.
    uniq = np.unique(a)
    integral = bool(np.all(np.isclose(uniq, np.round(uniq))))
    lo, hi = int(np.floor(a.min())), int(np.ceil(a.max()))
    if integral and (hi - lo) <= 14:
        bins = np.arange(lo - .5, hi + 1.5, 1)
    else:
        bins = np.linspace(float(np.percentile(a, 0.5)),
                           float(np.percentile(a, 99.5)), 30)
...
def p_castellvi(ax, rows):
    ...
    ax.set_yscale("log")
    ax.set_ylim(0.4, 60)
    ax.set_xlim(20, 100)          # a few segmentation outliers ran to 180mm
    ax.grid(alpha=.5, lw=.7)
...
def p_cooccur(ax, rows):
    ...
    def tl_odd(r):
        # n_rib_bearing was in here and flagged 797 of 802: the thoracic FOV is limited, so
        # a case shows 5-9 rib-bearing vertebrae for reasons of framing, not anatomy. And
        # the hypoplasia cut was 0.45 when the cohort MEDIAN ratio is 0.63 -- that called a
        # fifth of everyone hypoplastic. 0.30 is roughly the 8th centile.
        v = r.get("rib12_11_ratio_min")
        lowest = r.get("lowest_rib_bearing")
        return (bool(r.get("has_lumbar_rib"))
                or (isinstance(v, (int, float)) and v < 0.30)
                or (isinstance(lowest, str) and lowest not in ("T12", "")))
    def ls_odd(r):
        # foramina deliberately NOT used: the detector reads 3-vs-7 across one symmetric
        # sacrum, so it would inject noise into the very cell being tested.
        n = r.get("n_non_rib_bearing")
        return is_lstv(r) or (isinstance(n, (int, float)) and n != 5)
    rows = [r for r in rows if isinstance(r.get("n_non_rib_bearing"), (int, float))]
    ...
    aa, bb, cc, dd = m[1, 1], m[1, 0], m[0, 1], m[0, 0]
    orat = ((aa * dd) / (bb * cc)) if (bb and cc) else float("nan")
    title(ax, "Do both ends vary together?",
          f"observed {int(m[1,1])} vs {exp:.1f} if independent  ·  odds ratio {orat:.1f}")
...
def p_effects(ax, rows):
    ...
    """
    # identifiers are not measurements. `case` parses as an int and ranked FOURTH on the
    # first render -- it was reporting that LSTV cases have lower case numbers, which is a
    # fact about acquisition order, not anatomy.
    skip = {"case", "lstv_class", "has_l6", "foramen_best_slab_mm",
            "lowest_rib_bearing_id", "iliac_crest_at_id", "age"}
...
def main() -> int:
    ...
    fig = plt.figure(figsize=(15.5, 18.2))
    gs = fig.add_gridspec(4, 3, hspace=.52, wspace=.30,
                          left=.06, right=.97, top=.925, bottom=.05,
