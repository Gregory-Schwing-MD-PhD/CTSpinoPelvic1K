...
\emph{A proxy for the preoperative view.} No public preoperative lumbar CT cohort exists that we know of, and the largest public spine CT collection, VerSe, has 374 scans and no sacrum or pelvis (Table~\ref{tab:priorart}). This release is therefore the closest available stand-in for the view a lumbar case is planned on: the T12-to-S1 span with the lowest ribs and the pelvis; 377 of 802 records
acquired prone, the position of posterior lumbar surgery, with position recorded per
record; and classes for the anomalies that make level identification fail. It is a screening population at colonography dose, reconstructed with standard soft-tissue kernels (B30f, STANDARD, FC13) rather than the bone kernel of a surgical CT, and not a surgical series: it supports method
...
\emph{Future directions.} The scheme has been extended in place more than once (femora and S1, per-level ribs, lumbar ribs, hardware); imaging beyond this protocol, and the C2 anchor it cannot supply, are the natural next additions.
