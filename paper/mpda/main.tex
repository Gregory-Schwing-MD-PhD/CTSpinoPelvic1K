...
\emph{Measurement.} Dimensions and spinopelvic parameters come from code released with
the dataset. The sacral plate is fitted to its superior surface, and a fit whose normal
lies more than 60\si{\degree} off the cranial axis is rejected with the case reported as
missing: such a fit has found the near-vertical anterior face of the promontory rather
than the endplate, and the incidence that follows from it is arbitrary. The gate is on
the geometry rather than on the value, so an unusual patient survives it.
\toolkit{} is released separately
...
as a reusable implementation that reads each measure between landmarks in the vertebra's
own frame, fits each femoral head as a sphere on its articular surface, and resolves
structure names against the label scheme it detects in each volume. We found no
open-source implementation computing these parameters from CT segmentations.
...
published series rather than one also shows what a single curve conceals: for the same
measure and level the published means disagree by 21 to 69\%.
