...
\emph{Measurement.} Per-level dimensions come from code released with the dataset. The
spinopelvic parameters come from \toolkit{}
...
a separately released implementation that reads each measure between landmarks in the
vertebra's own frame rather than along the scanner's axes: the sacral plate by its
anterior and posterior corners, the construction pelvic incidence was defined with, and
each femoral head by a sphere fitted to its articular surface rather than a label
centroid. Every case carries the geometric identity check $\mathrm{PI}=\mathrm{SS}+
\mathrm{PT}$ and a plausibility gate, and a case failing either is reported as missing
with its reason rather than as a number. The toolkit resolves structure names against the
label scheme it detects in each volume: two schemes are in circulation for these data,
they differ in every identifier that matters, and resolving against the wrong one returns
a different bone rather than an error.
...
Vialle \emph{et al.}~\cite{vialle2005} (Table~\ref{tab:spinopelvic}). Pelvic incidence is the strongest of these checks because it is a morphological property
...
Pelvic incidence & 54.7\si{\degree} & 55 $\pm$ 10.6 \\
Pelvic tilt & 17.3\si{\degree} & 13 $\pm$ 6 \\
Sacral slope & 36.9\si{\degree} & 41.0 $\pm$ 8.4 \\
Lumbar lordosis & 52.7\si{\degree} & 60 $\pm$ 10 \\
...
\emph{Reference morphometry with its spread.} The values a surgeon consults for endplate and canal dimensions and pedicle width come
from cadaveric series of ten to thirty specimens\cite{panjabi1991,panjabi1992,berry1987}
and are redrawn in the textbooks as one curve per level.\cite{benzel2015} A standard error,
where one is reported, describes the mean and not the spread of individuals, so neither
tells the reader whether the patient in front of them is unusual. The same measures are
given here from 802 records with the distribution attached (Fig.~\ref{fig:levelatlas}),
reproducing the classical figures' pedicle widening into the lower lumbar spine and adding
the width of each distribution, the part needed to judge an individual. Drawing every
series the literature offers rather than one also shows what a single curve conceals: for
the same measure and level the published means disagree by 21 to 69\%, a spread comparable
to the between-patient spread they are used to interpret.
