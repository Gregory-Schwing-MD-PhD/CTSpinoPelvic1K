...
in any of them, however good the segmentation, and a segmenter trained on them inherits the
gap: faced with an enumeration anomaly it must shift the whole column by a level or absorb a
vertebra into its neighbor. The size of that problem has now been measured on CT, where
prior labeling methods assigned every vertebra correctly in 77\% of subjects and an
anomaly-aware method raised that to 99\%~\cite{veridah2026}. VerSe~\cite{verse2021,versedata2021}
makes the point most clearly (Table~\ref{tab:priorart}): it deliberately enriched for
anatomical variants and graded them by Castellvi, yet left the graded vertebrae fused to the
sacrum unsegmented.
...
annotation. A thirteenth thoracic vertebra, a lumbar rib and a \emph{stump rib} (a
hypoplastic twelfth rib, a cardinal indicator of a thoracolumbar transitional vertebra)
produce overlapping appearances that counting cannot resolve here.
...
excluded from sex-stratified measures rather than folded into ``unrecorded''. The lower age bound is the screening guideline
