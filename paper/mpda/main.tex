...
\newcommand{\has}{{\color{has}\rule[-0.55ex]{12mm}{2.3ex}}}
\newcommand{\qual}{{\color{qual}\rule[-0.55ex]{12mm}{2.3ex}}}
\newcommand{\no}{\hspace{12mm}}
\newcommand{\hd}[1]{\makebox[12mm]{#1}}
\usepackage{booktabs}
\usepackage{siunitx}
\usepackage{tikz}
...
\begin{table*}[t]
\caption{Public CT collections at the lumbosacral junction. Teal, class present; amber,
...
\begin{tikzpicture}[
  ...
  src/.style={bx=23mm, fill=black!5},
  proc/.style={bx=74mm, fill=blue!7},
  rec/.style={bx=17mm, fill=black!3},
  ver/.style={bx=9mm, fill=green!14, minimum height=7mm},
  det/.style={align=left, text width=61mm, anchor=west},
  gate/.style={bx=74mm, fill=orange!14},
  fin/.style={bx=74mm, fill=green!22},
  ar/.style={-{Stealth[length=4pt]}, semithick, black!70},
  ln/.style={semithick, black!70},
]
\node[src] (tcia)   at (-26, 0) {\textbf{TCIA CT Colonography}\\CT volumes};
\node[src] (spine)  at (  0, 0) {\textbf{CTSpine1K}\\vertebral labels};
\node[src] (pelvic) at ( 26, 0) {\textbf{CTPelvic1K}\\sacrum + hip labels};
\draw[ln] (tcia.south) -- (-26,-6.5) -- (26,-6.5) -- (pelvic.south);
\draw[ln] (spine.south) -- (0,-6.5);
\draw[ar] (0,-6.5) -- (0,-9.5);
\node[proc] (cross) at (0,-15.5) {\textbf{match each label set to the series it was drawn on}\\
  every patient was scanned prone \emph{and} supine; neither source published which};
\node[rec] (fused) at (-28.5,-31) {\textbf{fused}\\one series\\$n{=}342$};
\node[rec] (sep)   at ( -9.5,-31) {\textbf{separate}\\two series\\$n{=}351$};
\node[rec] (sonly) at (  9.5,-31) {\textbf{spine only}\\no pelvis\\$n{=}89$};
\node[rec] (ponly) at ( 28.5,-31) {\textbf{pelvis only}\\held back\\$n{=}20$};
\draw[ln] (cross.south) -- (0,-23);
\draw[ln] (-28.5,-23) -- (28.5,-23);
\foreach \x in {-28.5,-9.5,9.5,28.5} \draw[ar] (\x,-23) -- (\x,-26);
\node[proc] (fix) at (0,-47) {\textbf{correct errors in the source ground truth}\\
  split vertebrae, skipped levels, levels unlabeled inside the FOV:
  $n{=}112$ spine fixes / 103 cases, $n{=}13$ pelvic fixes / 12 cases};
\foreach \x in {-28.5,-9.5,9.5,28.5} \draw[ln] (\x,-36) -- (\x,-38.5);
\draw[ln] (-28.5,-38.5) -- (28.5,-38.5);
\draw[ar] (0,-38.5) -- (fix.north);
\node[proc] (dense) at (0,-63) {\textbf{pseudolabel the missing half --- asymmetrically, by design}\\
...
\node[ver] (v3) at (-32,-76)  {\textbf{v3}};
\node[ver] (v4) at (-32,-87)  {\textbf{v4}};
\node[ver] (v5) at (-32,-98)  {\textbf{v5}};
\node[ver] (v6) at (-32,-109) {\textbf{v6}};
\draw[ar] (dense.south) -- (0,-70) -- (-32,-70) -- (v3.north);
\draw[ar] (v3.south) -- (v4.north); \draw[ar] (v4.south) -- (v5.north); \draw[ar] (v5.south) -- (v6.north);
\foreach \y in {-76,-87,-98,-109} \draw[ln, black!35] (-27.5,\y) -- (-25.5,\y);
\node[det] at (-25,-76)  {added \textbf{femora} and the lower thoracic levels; carved \textbf{S1}
  out of the sacrum as its own class, the caudal anchor};
\node[det] at (-25,-87)  {added \textbf{ribs}: M\"oller's binary rib net unioned with
  TotalSegmentator, numbered from it; $n{=}152$ cases to human review};
\node[det] at (-25,-98)  {\textbf{lumbar ribs} relabeled to their own class rather than
  forced to be rib~12 ($n{=}16$ cases)};
\node[det] at (-25,-109) {added \textbf{surgical instrumentation} as its own classes
  ($n{=}11$ cases); renumbered 0068; corrected hip laterality ($n{=}22$)};
\node[gate] (qc) at (0,-124) {\textbf{release gates, in order --- a failure stops the chain}\\
  label shares its CT's affine, structures correctly sided $\rightarrow$ each rib
  articulates with the vertebra its number implies $\rightarrow$ measures in range};
\draw[ar] (v6.south) -- (-32,-115.5) -- (0,-115.5) -- (qc.north);
\node[fin] (rel) at (0,-138) {\textbf{802 released records}\\spine, pelvis, per-level ribs and
