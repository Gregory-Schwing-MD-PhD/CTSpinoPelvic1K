...
anatomy in front of the reader, on the abdominal and spine-limited images a surgeon actually
has, in the patients where counting fails. The cohort suits that question. Every record
...
so the thoracic spine enters from below and C2 is never present (Sec.~\ref{sec:applications}).
...
above pins the exact release measured in this manuscript (Sec.~\ref{sec:format}).
...
whose last rib was not visible, so it is recorded here as an observation from this
cohort. Since this is a colonography cohort, the lower ribs are routinely
