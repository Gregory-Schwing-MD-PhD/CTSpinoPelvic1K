...
Abdominopelvic computed tomography (CT) holds that span, the lowest ribs and the pelvis,
and stands in for the preoperative view. Where a
...
ribs and femora, lumbar levels anchored on the lowest rib-bearing vertebra and S1, and classes for a
sixth lumbar vertebra, a thirteenth thoracic vertebra, a separate S1 and lumbar ribs, so anomalies are recorded as themselves.

\textbf{Acquisition and Validation Methods:} Records pair TCIA CT~COLONOGRAPHY with
CTSpine1K~\cite{ctspine1k} vertebral and CTPelvic1K~\cite{ctpelvic1k} pelvic labels on the bone-richest acquisition, under a VerSe-native scheme. Validation: geometric invariants (802/802 pass); rib--vertebra incidence across 5{,}749 evaluable ribs (0.035\% offset); and spinopelvic measures matching published values.

\textbf{Data Format and Usage Notes:} Gzipped NIfTI image/label pairs with patient-grouped LSTV-stratified five-fold splits and a reference loader; archived at \doiurl{\datasetdoi}.

\textbf{Potential Applications:} Classifying a vertebra from local features;
updating cadaveric textbook morphometry;
spinopelvic assessment; variant studies; opportunistic screening; and, absent a public
...
99\%~\cite{veridah2026}. Its weights label L6 and T13 on CT, but its 1{,}536 training scans are in-house and unreleased: the capability exists as weights, not as data. VerSe~\cite{verse2021,versedata2021}
...
\emph{A proxy for the preoperative view.} We know of no public, labeled preoperative lumbar
CT cohort. In its absence this release is the closest available proxy for the field of view
a lumbar case is planned on: it holds the T12-to-S1 span with the lowest ribs and the
pelvis; 377 of its 802 records were acquired prone, the position of posterior lumbar
surgery, and the manifest records each record's position; and it labels the anomalies that
make level identification fail. It is a screening population imaged at colonography dose,
not a surgical series, so it supports method development for level identification and
instrumentation geometry rather than validation of a planning decision.
