...
\emph{Measurement.} Dimensions here come from code released with the dataset. A
reusable implementation that reads each between landmarks in the vertebra's own frame,
rather than along the scanner's axes, is released separately as \toolkit{}
(\ifreview the URL identifies the authors and is on the title page\else\url{https://github.com/OpenSpineConsortium/OpenSpineToolkit}\fi). The two agree
within a millimetre on canal depth; they differ most on posterior body height, where the
frame removes a tilt term worth several millimetres at L5.
...
\caption{Morphometry by level, as median, interquartile range and 5th--95th percentile, with $n$ at
right. (a) Endplate width. (b) Canal. (c) Pedicle width. Dashed lines are Panjabi's
cadaveric means for the same measures, the series the textbook figures redraw.}
...
from cadaveric series of twelve to thirty specimens\cite{panjabi1991,panjabi1992,berry1987}
