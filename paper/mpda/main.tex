...
% ---------------------------------------------------------------------------
\begin{abstract}
\textbf{Purpose:} Radiologic convention resolves vertebral numbering in an atypical spine by
counting caudally from C2 on whole-spine imaging, the reference standard for
numeration~\cite{lian2018}. Abdominal CT does not contain C2, and where a lumbosacral
...
support. Absent that, what remains is the sequence, which the reference standard establishes
by counting caudally from C2 on whole-spine imaging~\cite{lian2018}, unavailable in any
spine-limited acquisition. LSTV is common,
...
identifier this release keeps, so its L6 \emph{is} VerSe's. L6 is retained because a scheme
without it cannot record a six-lumbar spine at all.
...
that report them. That is a correction: through v5, \texttt{has\_l6} was true for one
record that contains no L6 and false for all eighteen that do, \texttt{has\_lumbar\_rib} was
false everywhere, and \texttt{n\_lumbar\_labels} read zero in 799 of 802 records. The fields
are rebuilt for this release by counting identifiers 25 and 74--75 in each volume, with
\texttt{lumbar\_rib\_side} added alongside them.
