...
Abdominopelvic computed tomography (CT) holds that span with the lowest ribs and the pelvis,
...
vertebra, a separate first sacral segment and lumbar ribs, so anomalies are recorded as themselves.

\textbf{Acquisition and Validation Methods:} Records pair TCIA CT~COLONOGRAPHY imaging with
CTSpine1K~\cite{ctspine1k} vertebral and CTPelvic1K~\cite{ctpelvic1k} pelvic labels on the acquisition with most bone, under a VerSe-native scheme. Validation: geometric invariants (802/802 pass); rib--vertebra incidence across 5{,}749 evaluable ribs (0.035\% offset); and agreement of spinopelvic measures with published values.

\textbf{Data Format and Usage Notes:} Gzipped NIfTI image/label pairs canonicalized to PIR, with patient-grouped LSTV-stratified five-fold splits and a reference loader; archived at \doiurl{\datasetdoi}.
...
angles supine; no held-out test set; ribs are triaged-review pseudolabels;
