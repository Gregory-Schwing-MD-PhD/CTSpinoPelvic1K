...
which reads each measure between landmarks in the vertebra's own frame rather than along
the scanner's axes: the sacral plate by its anterior and posterior corners, the
construction pelvic incidence was defined with, and each femoral head by a sphere fitted
to its articular surface. Every case carries the identity check
$\mathrm{PI}=\mathrm{SS}+\mathrm{PT}$ and a plausibility gate, and a case failing either
is reported as missing with its reason rather than as a number. We found no open-source
implementation computing these parameters from CT segmentations.
