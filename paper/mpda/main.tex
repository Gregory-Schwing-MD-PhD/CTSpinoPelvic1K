...
whole-spine imaging and counting caudally from C2~\cite{lian2018}. The imaging a lumbar
case is planned on is regional by guideline~\cite{acr2021} and never contains C2, and where
a lumbosacral
...
\textbf{Data Format and Usage Notes:} Gzipped NIfTI image/label pairs sharing one affine, canonicalized to PIR, with frozen patient-grouped LSTV-stratified five-fold splits and a reference loader; archived at \doiurl{\datasetdoi}.

\textbf{Potential Applications:} Classifying a vertebra from local features alone;
updating textbook morphometry from small cadaveric series;
...
guideline those are regional lumbar studies~\cite{acr2021,nass2013}, and whole-spine
coverage is reserved for trauma with an identified injury~\cite{tqip2018} and for deformity
radiographs. The cohort suits that question. Every record
...
Validation is layered: measurements on a corpus that has not established internal
consistency do not look broken, they look finished.
...
than adding them. In 8\ of these cases the femoral head that pelvic incidence and tilt are
