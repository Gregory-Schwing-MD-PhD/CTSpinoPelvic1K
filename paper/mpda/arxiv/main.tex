...
standard counts caudally from C2 on whole-spine imaging~\cite{lian2018}; however, a lumbar surgical case is planned on a lumbar study~\cite{acr2021,acrmri2023}, T12 to S1, without C2.
...
The project began with a surgeon's question, whether a vertebra can be named from the anatomy in front of the reader, on the images a lumbar surgical case is actually planned on. By guideline those are lumbar studies~\cite{acr2021,nass2013,acrmri2023}, T12 to S1, and whole-spine
...
Stratification is on the transitional subtype rather than a binary LSTV flag, so every validation fold carries three sacralization and three lumbarization records. With 15 of
...
\emph{Limitations.} The cohort comes from one acquisition protocol of a colorectal-screening population aged 50 and over, so how well it generalizes to a pathologic population is untested. Human review of the rib layer was triaged by an automated rule, so it guarantees only the absence of the failures that rule detects. 
...
\bibitem{poolman2023}
A.~M. Poolman, Q.~Wessels, A.~Van~Schoor, and N.~Keough,
...
\bibitem{nagata2025}
S.~Nagata, C.~Yamada, M.~Sawano, et~al.,
