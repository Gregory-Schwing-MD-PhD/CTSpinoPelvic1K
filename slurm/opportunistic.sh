...
#SBATCH --job-name=ctsp_opportunistic
#SBATCH -q primary
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH --output=logs/opportunistic_%j.out
#SBATCH --error=logs/opportunistic_%j.out
set -uo pipefail
...
singularity exec --bind "$(pwd)":/w --pwd /w \
  ...
  python3 scripts/extract_opportunistic.py \
