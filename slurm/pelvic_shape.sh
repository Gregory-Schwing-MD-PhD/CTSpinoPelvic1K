#!/usr/bin/env bash
#SBATCH --job-name=ctsp_pelvicshape
#SBATCH -q primary
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH --output=logs/pelvicshape_%j.out
#SBATCH --error=logs/pelvicshape_%j.out
set -uo pipefail
...
singularity exec --bind "$(pwd)":/w --pwd /w \
  ...
  python3 scripts/extract_pelvic_shape.py --labels data/v5_final \
