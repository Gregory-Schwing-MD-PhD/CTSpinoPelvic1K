"""
test_merge_labels_anchor.py — the rib anchor (VerSe T12=19 -> class 31 (native, contiguous)) is
...
def _spine():
    ...
    sp[0, 0, 0] = 19                       # T12 (last rib-bearing vertebra)
    sp[1, 0, 0] = 20                       # L1
    sp[2, 0, 0] = 21
    sp[3, 0, 0] = 22
    sp[4, 0, 0] = 23
...
def test_fused_retains_anchor_and_preserves_regions():
    ...
    assert r[0, 0, 0] == 31               # VerSe 19 (T12) -> 31 (per-level, contiguous)
...
def test_partial_spine_only_anchor_and_ignore():
    ...
    assert r[0, 0, 0] == 31               # T12 retained
...
def test_anchor_not_overwritten_by_sacrum_fill():
    ...
    r = merge_labels(_w(sp), None, shape)
    assert r[0, 0, 0] == 31
    assert int((r == 31).sum()) == 1      # exactly the one T12 voxel
...
def test_cervical_and_thoracic_retained_from_gt():
    ...
    # T12 native (VerSe 19 -> 31) and L1-L6 unchanged (VerSe 20-25 -> 1-6).
    shape = (10, 4, 4)
    sp = np.zeros(shape, np.int16)
    sp[0, 0, 0] = 1            # C1   -> 13
    sp[1, 0, 0] = 7            # C7   -> 19
    sp[2, 0, 0] = 8            # T1   -> 20
    sp[3, 0, 0] = 18           # T11  -> 30
    sp[4, 0, 0] = 19           # T12  -> 31 (native, contiguous; anchor derived)
    sp[5, 0, 0] = 20           # L1   -> 1
    sp[6, 0, 0] = 25           # L6   -> 6
    ...
    assert r[4, 0, 0] == 31    # T12 -> per-level (column contiguous 20-32)
    assert r[5, 0, 0] == 1     # L1
    assert r[6, 0, 0] == 6     # L6
