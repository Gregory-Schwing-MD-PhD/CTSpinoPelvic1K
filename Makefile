...
# NOTE: no inline comments on value-bearing vars — Make keeps the spaces before
# '#' in the value, which breaks the comma-separated sbatch --export list.
# MODE: test = fast smoke on a few cases; production = full run.
MODE           ?= production
MANIFEST       ?=
NIFTI_DIR      ?=
PROP_OUT_DIR   ?=
...
DRY_RUN        ?= 0
PREDICT_FUSED  ?= 0
# Optional SLURM dependency for chaining jobs overnight, e.g.
#   make eval-vs-manual SBATCH_DEP=afterok:<jobid>
SBATCH_DEP     ?=
DEP            := $(if $(strip $(SBATCH_DEP)),--dependency=$(strip $(SBATCH_DEP)),)
SKIP_DOWNLOAD  ?= 0
PSEUDO_LIMIT   ?= 0
...
pseudolabel:  ## Stage 3.5 — complete partial cases via out-of-fold nnU-Net (DRY_RUN=1 to plan)
	...
	sbatch $(DEP) --export=ALL,SIF_PATH=$(CONTAINER),NNUNET_SIF=$(NNUNET_SIF),NNUNET_RESULTS=$(NNUNET_RESULTS),HF_EXPORT_DIR=$(HF_EXPORT_DIR),PSEUDO_OUT_DIR=$(PSEUDO_OUT_DIR),MODELS_CONFIG=$(MODELS_CONFIG),DRY_RUN=$(DRY_RUN),SKIP_DOWNLOAD=$(SKIP_DOWNLOAD),PSEUDO_LIMIT=$(PSEUDO_LIMIT),PREDICT_FUSED=$(PREDICT_FUSED),HF_TOKEN=$(HF_TOKEN) \
...
eval-vs-manual: check-container  ## Quantify model accuracy vs MANUAL ground truth on the scoped manual side (CPU)
	...
	sbatch $(DEP) --export=ALL,SIF_PATH=$(CONTAINER),HF_EXPORT_DIR=$(HF_EXPORT_DIR),PRED_DIR=$(PRED_DIR),MODELS_CONFIG=$(MODELS_CONFIG),EVAL_CSV=$(EVAL_CSV),EVAL_WORKERS=$(EVAL_WORKERS),EVAL_NO_ASSD=$(EVAL_NO_ASSD),EVAL_INCLUDE_FUSED=$(EVAL_INCLUDE_FUSED) \
