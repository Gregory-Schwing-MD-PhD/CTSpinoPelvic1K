...
\floatconts{fig:visual_abstract}
  {\caption{\textbf{How \dataset{} is built.}
    ...
    under the ignore-label protocol (all 1{,}153 cases). Both partial
    configurations are then completed out-of-fold by that model: the 782
    spine-anchored cases have their pelvis filled (radiologist spine GT kept,
    prioritized over pelvic-only), and the 20 patients with no spine acquisition
    receive a pseudolabelled spine that a radiologist verifies for enumeration
    errors---802 released volumes in total. Ribs are then pseudolabelled with
    \textsc{TotalSegmentator} and added in v3. The page stacks are placeholders
    for 2-D CT-with-mask overlay renders; the count below each is the number of
    volumes in that category.}}
  {\resizebox{\textwidth}{!}{%
  ...
  \node[hdr=vacpseudo] at (14.5, 5.9) {\textsc{Pseudolabel}\\[-1pt]\footnotesize fill the missing region};
  ...
  \node[lbl=vacgray, font=\sffamily\scriptsize, align=center] at (12.2,-3.6)
...
Spine annotations take priority over pelvic-only ones: every patient
with a spine acquisition (782 in total) is represented by that scan with
its pelvis model-completed. Only the 20 patients with no spine
acquisition at all receive a model-completed spine, and \emph{every}
such case is reviewed by a radiologist specifically for enumeration
errors on the spinal instance segmentation before release. Across both
routes, all 802 released patient volumes carry a complete spine+pelvis
labelmap with no ignore-label voxels; the only model-generated labels
are pelves on spine-annotated scans plus the 20 radiologist-verified
pelvic-only spines.
