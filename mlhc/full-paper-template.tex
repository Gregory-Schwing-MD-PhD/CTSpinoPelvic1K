...
\newcommand{\vacph}[1]{\parbox[c][2.2cm][c]{1.75cm}{\centering
...
\floatconts{fig:visual_abstract}
  {\caption{\textbf{How \dataset{} is built.}
    ...
    scan are merged (brace) into a \textsc{fused} case; masks on \emph{different}
    scans stay \textsc{separate} ($\rightarrow$) as spine-only and pelvic-only
    volumes. All configurations train an nnU-Net v2 under the ignore-label
    protocol (all 1{,}153 cases). The fused and spine-only cases (782) are then
    pelvis-completed out-of-fold by that model; ribs are pseudolabelled with
    \textsc{TotalSegmentator} and added in v3. The page stacks are placeholders
    for 2-D CT-with-mask overlay renders. Counts are shown at each stage.}}
  {\resizebox{\textwidth}{!}{%
  \begin{tikzpicture}[x=1cm, y=1cm]
  \useasboundingbox (-3.9,-5.7) rectangle (20.1,6.2);   % padding -> margins + caption gap

  % ── phase headers (kept well above the stacks) ──
  \node[hdr=vacgray]   at (0,    5.5) {\textsc{Source masks}\\[-1pt]\footnotesize two TCIA datasets};
  \node[hdr=vacgray]   at (5,    5.5) {\textsc{Patient-anchored}\\[-1pt]\footnotesize match (bone-HU)};
  \node[hdr=vactrain]  at (9.6,  5.5) {\textsc{Partial-annotation}\\[-1pt]\footnotesize training};
  \node[hdr=vacpseudo] at (13.8, 5.5) {\textsc{Pseudolabel}\\[-1pt]\footnotesize complete the pelvis};
  \node[hdr=vacrib]    at (17.8, 5.5) {\textsc{+ Ribs}\\[-1pt]\footnotesize TotalSegmentator, v3};

  % ── (1) source mask stacks (labels to the LEFT, clear of the merge) ──
  \pagestack{spine}{0}{2.5}{vacspine}{\vacSpine}
  \node[lbl=vacspine, anchor=east, align=right] at (-1.2,2.5)
    {CTSpine1K\\spine GT\\\textbf{784 masks}};
  \pagestack{pelv}{0}{-2.5}{vacpelv}{\vacPelvis}
  \node[lbl=vacpelv, anchor=east, align=right] at (-1.2,-2.5)
    {CTPelvic1K\\pelvic GT\\\textbf{714 masks}};

  % ── (2) match: } brace -> fused; -> arrows -> separate ──
  \pagestack{sonly}{5}{3.4}{vacspine}{\vacSpineOnly}
  \pagestack{fused}{5}{0}{vacfuse}{\vacFused}
  \pagestack{ponly}{5}{-3.4}{vacpelv}{\vacPelvicOnly}
  ...
  \draw[line width=0.8pt, draw=vacfuse!55!black] (spine.east) -- (2.4,2.5);
  \draw[line width=0.8pt, draw=vacfuse!55!black] (pelv.east)  -- (2.4,-2.5);
  ...
  \node[lbl=vacfuse, font=\sffamily\scriptsize, fill=white, inner sep=1pt] at (3.55,1.25) {same scan};
  ...
  \node[lbl=vacgray, font=\sffamily\scriptsize, fill=white, inner sep=1pt] at (2.55,3.75) {different scans};
  ...
  \pagestack{dense}{13.8}{0}{vacpseudo}{\vacDense}
  \node[lbl=vacpseudo, align=center] at (13.8,-1.95) {dense spine\,+\,pelvis\\\textbf{782}};
  ...
  \node[lbl=vacgray, font=\sffamily\scriptsize, align=center] at (11.7,-3.2)
    {pelvic-only 371\\held out};

  % ── (5) ribs (TotalSegmentator) ──
  \pagestack{ribs}{17.8}{0}{vacrib}{\vacRibs}
  \node[lbl=vacrib, align=center] at (17.8,-1.95) {spine\,+\,pelvis\,+\,ribs\\\textbf{782}};
