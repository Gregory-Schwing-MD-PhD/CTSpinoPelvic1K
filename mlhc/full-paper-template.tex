...
% ── Visual-abstract figure: colours, styles, and book-page / anatomy macros ──
...
\floatconts{fig:visual_abstract}
  {\caption{\textbf{How \dataset{} is built.}
    ...
    scan merge into a \textsc{fused} case (\}); masks on \emph{different} scans
    stay \textsc{separate} ($\rightarrow$) as spine-only and pelvic-only volumes.
    All configurations train an nnU-Net v2 under the ignore-label protocol (all
    1{,}153 cases). The fused and spine-only cases (782) are then pelvis-completed
    out-of-fold by that model; ribs are added in v3. Counts are shown at each
    stage.}}
  {\resizebox{\textwidth}{!}{%
  \begin{tikzpicture}[x=1cm, y=1cm]
  \node[hdr=vacgray]   at (0,    4.2) {\textsc{Source masks}\\[-1pt]\footnotesize two TCIA datasets};
  \node[hdr=vacgray]   at (5,    4.2) {\textsc{Patient-anchored}\\[-1pt]\footnotesize match (bone-HU)};
  \node[hdr=vactrain]  at (9.6,  4.2) {\textsc{Partial-annotation}\\[-1pt]\footnotesize training};
  \node[hdr=vacpseudo] at (13.8, 4.2) {\textsc{Pseudolabel}\\[-1pt]\footnotesize complete the pelvis};
  \node[hdr=vacrib]    at (17.8, 4.2) {\textsc{+ Ribs}\\[-1pt]\footnotesize v3};

  \pagestack{spine}{0}{2.2}{vacspine}   \spineicon{0}{2.2}{vacspine}
  \node[lbl=vacspine] at (0,0.65) {CTSpine1K\\spine GT};
  \node[cnt]          at (0,0.15) {784 masks};
  \pagestack{pelv}{0}{-2.2}{vacpelv}    \pelvisicon{0}{-2.2}{vacpelv}
  \node[lbl=vacpelv]  at (0,-3.75) {CTPelvic1K\\pelvic GT};
  \node[cnt]          at (0,-4.25) {714 masks};

  \pagestack{fused}{5}{0}{vacfuse}
    \spineicon{4.65}{0}{vacspine}\pelvisicon{5.45}{-0.15}{vacpelv}
  \node[lbl=vacfuse]  at (5,-1.55) {Fused (same CT)};
  \node[cnt]          at (5,-2.05) {342};
  \pagestack{sonly}{5}{2.5}{vacspine}   \spineicon{5}{2.5}{vacspine}
  \node[cnt, anchor=west] at (6.25,2.9) {Spine-only \textbf{440}};
  \pagestack{ponly}{5}{-3.0}{vacpelv}   \pelvisicon{5}{-3.0}{vacpelv}
  \node[cnt, anchor=west] at (6.25,-2.6) {Pelvic-only \textbf{371}};

  \draw[decorate, decoration={brace, amplitude=8pt, mirror}, line width=1pt, draw=vacfuse!70!black]
    ($(spine.east)+(0.9,0.1)$) -- ($(pelv.east)+(0.9,-0.1)$);
  \draw[flow, draw=vacfuse!70!black] ($(spine.east)+(0.95,0)$) -- ++(0.35,0) |- (fused.west);
  \node[lbl=vacfuse, font=\sffamily\scriptsize] at (2.95,0.95) {same scan\\$\Rightarrow$ merge};
  \draw[flow] (spine.east) to[out=0,in=180] (sonly.west);
  \draw[flow] (pelv.east)  to[out=0,in=180] (ponly.west);
  \node[lbl=vacgray, font=\sffamily\scriptsize] at (2.95,3.05) {different scans\\$\rightarrow$ separate};
  ...
  \pagestack{dense}{13.8}{0}{vacpseudo}
    \spineicon{13.45}{0}{vacpseudo}\pelvisicon{14.25}{-0.15}{vacpseudo}
  \node[lbl=vacpseudo] at (13.8,-1.55) {dense spine\,+\,pelvis};
  \node[cnt]           at (13.8,-2.05) {782};
  \draw[flow, draw=vacpseudo!70!black] (train.east) -- (dense.west)
    node[midway, above, font=\sffamily\scriptsize, text=vacpseudo!60!black, align=center]
    {fused 342 + spine-only 440\\[-1pt]\textbf{= 782}};
  \node[lbl=vacgray, font=\sffamily\scriptsize, align=center] at (11.7,-2.5)
    {pelvic-only 371\\held out};

  \pagestack{ribs}{17.8}{0}{vacrib}
    \spineicon{17.45}{0}{vacrib}\pelvisicon{18.25}{-0.15}{vacrib}\ribsicon{17.8}{0}{vacrib}
  \node[lbl=vacrib] at (17.8,-1.55) {spine\,+\,pelvis\,+\,ribs};
  \node[cnt]        at (17.8,-2.05) {782};
  \draw[flow, draw=vacrib!70!black] (dense.east) -- (ribs.west)
    node[midway, above, font=\sffamily\scriptsize, text=vacrib!60!black] {+ ribs};
  \end{tikzpicture}}}
\end{figure*}
% ── END FIGURE ───────────────────────────────────────────────────────────────
...
Identity is an exact match on the DICOM \texttt{PatientID} carried
in every mask filename; the only difference between the two releases
is zero-padding, normalized by stripping leading zeros from the
final UID component (Eq.~\ref{eq:canon}). This resolves 825/825
patients with no heuristic, and we do not claim it as a
...
fusion demonstrated here. The underlying primitives---UID
canonicalization for identity resolution, and tissue-HU-based
...
Both CTSpine1K and CTPelvic1K mask filenames encode the TCIA
\texttt{PatientID} DICOM tag but not the \texttt{SeriesInstanceUID}.
Both UIDs carry zero-padded last components that vary between
dataset releases, causing naive string matching to fail.
We establish patient identity via canonical UID matching:
\begin{equation}
\text{canon}(u) = \text{prefix}(u) \,\|\, \text{int}(\text{suffix}(u))
\label{eq:canon}
\end{equation}
where $\text{int}(\cdot)$ strips leading zeros from the final UID
component. Identity is \emph{algebraically exact}---no heuristic
required---and we do not claim it as a contribution; it merely bounds
the search to a patient. PatientDB (schema in
Appendix~\ref{app:patientdb}) organizes all data using canonical
patient UID as the primary key, and resolves \textbf{825/825
patients (100\%)}. The substantive problem is the Phase~2 series
alignment below: the \texttt{SeriesInstanceUID} did not survive the
DICOM-to-NIfTI conversion, so each mask must still be placed onto
one of its patient's several scans.
...
PatientDB organizes all data using canonical patient UID as the
primary key, with five entities:
\texttt{PATIENT} (root entity indexed by canonical UID);
