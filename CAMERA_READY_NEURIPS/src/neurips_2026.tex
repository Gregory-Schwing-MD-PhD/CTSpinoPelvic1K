...
% Patrick Schwing's affiliation, supplied by the corresponding author on 2026-09-25
% (it is recorded in no project file).
\newcommand{\PatrickAffil}{School of Medicine \\
...
epoch and fluctuates (29--100\% within one fold over the
last 100 epochs); the fold-level means are the statistic.
With the same data, folds and capacity, adding an L6 class
does not remove the L5/L6 collision; merging does. This is
a limit of dense segmentation at this LSTV sample size (11
lumbarization patients in the training pool), not an
impossibility.

\paragraph{Residual upper-lumbar caudal-shift cascade.}
The merge restores the bottom-most boundary (Block~1) but
leaves a residual caudal shift in the upper lumbars on lumb
cases: per-class DSC L1\,=\,$0.872$, L2\,=\,$0.611$,
L3\,=\,$0.377$, L4\,=\,$0.213$ (Table~\ref{tab:baseline};
two of the three cases shift, the third scores L1--L3 above
$0.98$). \sota{}'s shift sits at the bottom (no L6 class),
ours through the upper lumbars, so on per-class L2--L4
\sota{} is better, trading upper-lumbar precision
for bottom-anchor correctness. Junction-DSC, centered on
L5/S1, closes the surgically critical gap (+58.9~pts on
the one scorable case) regardless; the upper cascade needs
a sequence predictor with count constraints (contribution
iv): the merge is necessary but insufficient.
...
needed to count. This is the case for \VERIDAH{}'s
two-stage design: the merge addresses class imbalance;
telling a 4-vertebra spine's last lumbar from a 5-vertebra
spine's needs a component with global structure.
...
Any spine segmenter lacking an L6 class shifts labels on
lumbarization, and \S\ref{sec:unmerged} shows that adding
the class at this sample size does not remove it: the gap
is the symptom, not a model-specific bug. The 100\,GB patch
budget requires H200-class hardware.

\paragraph{Future work.}
\textbf{(1)~\VerSe{} expansion via sacrum
pseudolabeling.} \VerSe{}~\citep{Sekuboyina2021VerSe:Images}
labels L6 (label~25) in 460 CT volumes but has no pelvis;
sacrum pseudolabeling with our segmenter (sacral DSC
$0.969 \pm 0.002$, 5-fold) would give a $1{,}613$-volume
spinopelvic cohort.
\textbf{(2)~Two-stage integration with \VERIDAH{}}, our
segmenter replacing its \VerSe{}-derived CT upstream.
...
checkpoints over one 174-case test set (15 case-fold pairs
for lumbarization and 5 for sacr\_count in the voxel
confusion, the test-split $n$ for case-level Dice), and the
Wilson intervals for strata of 1--7 cases span 21--65\% to
100\%. The
...
well-formed, caudally shifted prediction on a
5--35\%-prevalence variant gives no error signal and
reproduces the pattern of the wrong-level miscount
downstream. Any model claiming readiness for surgical
planning should report per-subgroup DSC and L5/S1
confusion on a stratified LSTV cohort, with each
stratum's $n$.
...
(current version v11,
\href{https://doi.org/10.5281/zenodo.22797014}{10.5281/zenodo.22797014};
the experiments ran on the April 2026 release, \S\ref{sec:dataset}) with core and
...
trained with the released trainer and inference code at
\url{https://github.com/OpenSpineConsortium/spinopelvic-seg}
(commit \texttt{50b209f}); the data-construction pipeline,
the evaluation protocol and the \TS{} benchmark script are at
...
(NIGMS F30GM147909) during the first months of this work;
the work received no other funding. Imaging: The Cancer
Imaging Archive; source annotations: the CTSpine1K and
CTPelvic1K investigators. No conflicts of interest.
\end{ack}
